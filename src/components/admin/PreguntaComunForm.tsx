...
const PreguntaComunForm: React.FC<PreguntaComunFormProps> = ({
  ...
  const updateBlock = (
      ...
      if (context === 'common') {
          setCommonStatement(prev => prev.map(b => b.id === blockId ? { ...b, content: val } : b));
      } else if (subQId) {
          setSubQuestions(prev => prev.map(q => 
            q.tempId === subQId 
                ? { ...q, specificStatement: q.specificStatement.map(b => b.id === blockId ? { ...b, content: val } : b) } 
                : q
        ));
      }
  };
  ...
  const removeBlock = (
      ...
      if (context === 'common') {
          ...
          setSubQuestions(prev => prev.map(q => 
            q.tempId === subQId 
                ? { ...q, specificStatement: q.specificStatement.filter(b => b.id !== blockId) } 
                : q
        ));
      }
  };
  ...
  const triggerImageUpload = (context: 'common' | 'specific', subQId?: string) => {
  ...
  const updateSubQuestionField = (tempId: string, field: keyof SubPregunta, value: any) => {
    setSubQuestions(prev => prev.map(q => 
  ...
  const updateAlternative = (tempId: string, altIndex: number, field: string, value: any) => {
    setSubQuestions(prev => prev.map(q => {
        ...
        <div className="border border-primary rounded-lg p-6 bg-white shadow-sm">
            ...
            {isEmpty ? (
                <div className="border-2 border-dashed border-gray-200 rounded-lg h-32 flex flex-col items-center justify-center text-center bg-gray-50/50">
                    <p className="text-gray-500 text-sm font-medium mb-1">No hay elementos en {label.toLowerCase()}</p>
                       ...
                       <div key={block.id} className="relative group w-full">
                           ...
                           {block.type === 'text' ? (
                               <div className="quill-editor-container border border-gray-200 rounded-lg overflow-hidden">
                                   <TiptapEditor
                                     value={block.content}
                                     onChange={(val) => updateBlock(val, block.id, context, subQId)}
                               ...
                               <div className="border rounded-lg p-4 bg-gray-50 flex justify-center items-center">
                                   <img src={block.content} alt="Content" className="max-h-64 rounded shadow-sm" />
                               </div>
                           )}
                       </div>
                   ))}
    ...
    <div className="space-y-8 pb-20">
        ...
        <div className="space-y-2">
             <div className="flex items-center gap-2 mb-2 text-indigo-800">
                ...
                <h3 className="text-lg font-bold">Enunciado Común (Contexto Compartido)</h3>
            </div>
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <h3 className="text-gray-700 font-bold text-lg">Preguntas Asociadas ({subQuestions.length})</h3>
            </div>

            {subQuestions.map((q, index) => (
                <div key={q.tempId} className={`border rounded-xl bg-white shadow-sm transition-all ${q.isExpanded ? 'border-primary ring-1 ring-blue-100' : 'border-gray-200'}`}>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="font-medium text-gray-700 truncate max-w-md">
                                {q.specificStatement.length > 0
                                    ? <span className="text-green-600 font-bold">Con contenido ({q.specificStatement.length} bloques)</span> 
                                    : <span className="text-gray-400 italic">Nueva pregunta...</span>}
                            </span>
                        </div>
                        <div className="flex items-center gap-3">
                            <button 
                                onClick={(e) => { e.stopPropagation(); removeSubQuestion(q.tempId); }}
                                className="text-red-500 hover:bg-red-50 p-1 rounded transition-colors"
                            >
                                <TrashIcon className="w-5 h-5" />
                            </button>
                            {q.isExpanded ? <ChevronUpIcon className="w-5 h-5 text-gray-400" /> : <ChevronDownIcon className="w-5 h-5 text-gray-400" />}
                        </div>
                    </div>
                    ...
                    {q.isExpanded && (
                        <div className="p-6 border-t border-gray-100 space-y-6">
                                    ...
                                    <div className="w-full md:w-3/4">
                                        ...
                                        <div className="relative">
                                            <select 
                                                ...
                                                onChange={(e) => updateSubQuestionField(q.tempId, 'clasificacionId', Number(e.target.value))}
                                            >
                            ...
                            <div className="border border-gray-200 rounded-lg p-6 bg-white shadow-sm">
                                        ...
                                        <div key={alt.id} className="flex gap-4 items-center">
                                            ...
                                            <div className="flex-1">
                                                <TiptapEditor 
                                                    ...
                                                    placeholder={`Opción...`}
                                                    borderColor={alt.esCorrecta ? 'border-green-400' : 'border-gray-200'}
                                                />
                                            </div>
                             ...
                             {/* Sustento */}
                             <div>
                                <label className="block text-xs font-bold text-gray-600 mb-2 uppercase">Sustento (Opcional)</label>
                                <TiptapEditor 
                                    value={q.sustento}
                                    onChange={(val) => updateSubQuestionField(q.tempId, 'sustento', val)}
        ...
        <div className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 p-4 shadow-lg z-20 flex justify-end gap-4 md:px-10">
            <button onClick={onCancel} className="text-gray-600 hover:text-gray-800 font-medium px-4">
                Cancelar
            </button>
            <button 
                onClick={handleSaveAll}
                disabled={saving}
                className={`bg-primary text-white px-8 py-2 rounded shadow hover:bg-secondary font-medium transition-colors flex items-center ${saving ? 'opacity-70 cursor-wait' : ''}`}
            >
