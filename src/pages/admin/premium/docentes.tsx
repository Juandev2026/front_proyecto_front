    ...
    try {
      ...
      if (formData.modalidadId) {
        const alreadyAdded = finalUserExamenes.some(
          ex => ex.modalidadId === Number(formData.modalidadId)
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="border border-gray-200 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-4">
                    <svg className="w-5 h-5 text-[#002B6B]" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div className="border border-gray-200 rounded-xl p-6 relative">
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <h4 className="font-bold text-[#002B6B] text-lg">Información personal</h4>
                  {(() => {
                    const status = calculateUserStatus(selectedUserDetail.fechaExpiracion);
                    return (
                      <span className={`absolute right-6 top-6 px-3 py-1 rounded-lg text-sm font-bold ${getStatusColor(status)}`}>
                        {status}
                      </span>
                    );
                  })()}
                ...
                <div className="grid grid-cols-2 gap-y-6">
                  <div className="text-center border-r border-gray-100">
                    <p className="text-[#002B6B] font-bold text-sm mb-1">Nombre</p>
                    <p className="text-gray-700 font-medium">{selectedUserDetail.nombreCompleto}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-[#002B6B] font-bold text-sm mb-1">Correo:</p>
                    <p className="text-gray-700 font-medium break-all">{selectedUserDetail.email}</p>
                  </div>
                  <div className="col-span-2 text-center mt-2">
                    <p className="text-[#002B6B] font-bold text-sm mb-1">Teléfono:</p>
                    <p className="text-gray-700 font-medium text-lg tracking-wider">{selectedUserDetail.celular || 'N/A'}</p>
                  </div>
                </div>

                <div className="mt-8 flex justify-center gap-12 text-center">
                  <div>
                    <p className="text-[#002B6B] font-bold text-sm mb-1">Region</p>
                    <p className="text-gray-800 font-bold uppercase">{selectedUserDetail.region?.nombre || '-'}</p>
                  </div>
                  <div>
                    <p className="text-[#002B6B] font-bold text-sm mb-1">Institución:</p>
                    <p className="text-gray-800 font-bold">{selectedUserDetail.ie || '-'}</p>
                  </div>
                </div>

                <div className="mt-8">
                  <p className="text-[#002B6B] font-bold text-sm mb-2">Observaciones:</p>
              ...
              <div className="border border-gray-200 rounded-xl p-6">
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <h4 className="font-bold text-[#002B6B] text-lg">Información académica</h4>
                </div>
              ...
              <div className="border border-gray-200 rounded-xl p-6">
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <h4 className="font-bold text-[#002B6B] text-lg">Subscripciones</h4>
                </div>

                <div className="space-y-4">
                  {tiposAcceso.map(acceso => {
                    const hasAccess = selectedUserDetail.accesoIds?.includes(Number(acceso.id));
                    if (!hasAccess) return null;
                    return (
                      <div key={acceso.id} className="border border-gray-200 rounded-xl p-4 flex justify-between items-center bg-white shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div className="text-right">
                          <p className="text-gray-400 text-[10px] font-medium uppercase">Fecha de expiración:</p>
                          <p className="text-gray-700 font-bold text-sm">
                            {selectedUserDetail.fechaExpiracion ? new Date(selectedUserDetail.fechaExpiracion).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' }).replace(/\//g, '-') : '-'}
                          </p>
                        </div>
                      </div>
                    );
                  })}
                  {(!selectedUserDetail.accesoIds || selectedUserDetail.accesoIds.length === 0) && (
                    <p className="text-center text-gray-400 italic py-2">No tiene suscripciones activas</p>
                  )}
                </div>
              </div>
              ...
              <div className="border border-gray-200 rounded-xl p-6 text-center">
                <div className="flex items-center justify-center gap-2 mb-4">
                  ...
                  <h4 className="font-bold text-[#002B6B] text-lg">Fechas importantes</h4>
                </div>

                <p className="text-[#002B6B] font-bold text-sm mb-1">Fecha de creación:</p>
                <p className="text-gray-700 font-medium text-lg">
                  {selectedUserDetail.fechaCreacion ? selectedUserDetail.fechaCreacion.split('T')[0] : selectedUserDetail.fecha_creacion ? selectedUserDetail.fecha_creacion.split('T')[0] : '-'}
                </p>
              </div>
