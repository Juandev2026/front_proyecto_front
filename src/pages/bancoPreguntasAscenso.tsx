      ...
      if (stored) {
        try {
          ...
          const modalities = Array.from(
            new Map(parsed.map((e) => [e.modalidadId, e.modalidadId])).values()
          );
      ...
      <div className="w-full space-y-6">
        ...
        <div className="text-center py-4">
          ...
          <p className="text-[#A3AED0] text-base mt-1 font-medium">
            Puedes seleccionar el año de su preferencia
          </p>
        </div>
                ...
                if (selectedModalidadId && selectedNivelId && anio) {
                  try {
                    ...
                    const exams = JSON.parse(
                      localStorage.getItem('loginExamenes') || '[]'
                    ) as any[];
                    const matchingExams = exams.filter(
                      (e) =>
                        ...
                        (selectedEspecialidadId
                          ? e.especialidadId === Number(selectedEspecialidadId)
                          : true)
                    );
                    ...
                    console.log(
                      'Debug - Final ClasificacionIds:',
                      clasificacionIds
                    );

                    const questions =
                      await estructuraAcademicaService.getPreguntas(
                    ...
                    const metadata = {
                      modalidad: modalidadesData.find(
                        (m) => m.id === Number(selectedModalidadId)
                      )?.nombre,
                      nivel: nivelesData.find(
                        (n) => n.id === Number(selectedNivelId)
                      )?.nombre,
