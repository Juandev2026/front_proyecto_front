...
import {
  contenidoIntroductorioService,
  ContenidoIntroductorio,
} from '../services/contenidoIntroductorioService';
import {
  seccionRecursosService,
  SeccionAnidada,
} from '../services/seccionRecursosService';
...
const RecursosAscensoPage = () => {
  ...
  const stats = {
    ...
    archivos: sections.reduce(
      (acc, s) =>
        acc +
        s.subSecciones.reduce(
          (subAcc, sub) => subAcc + (sub.recurso?.length || 0),
          0
        ),
      0
    ),
  ...
  return (
    <>
      <PremiumLayout
        title="Recursos Ascenso"
        breadcrumb="Pages / Recursos Ascenso"
      >
        <Head>
        ...
        <div className="w-full space-y-6">
          ...
          <div className="w-full bg-[#4790FD] rounded-xl py-3 md:py-4 px-6 text-white text-center shadow-lg">
            ...
            <p className="text-xs md:text-sm font-bold text-gray-200 mt-0.5 uppercase tracking-wider">
              Ascenso Docente
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 md:gap-6">
            ...
            <div className="bg-white rounded-xl p-3 md:p-6 shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center">
              <span className="text-xl md:text-3xl font-bold text-[#4790FD]">
                {stats.secciones}
              </span>
              <span className="text-[10px] md:text-sm text-gray-500 font-medium mt-1">
                Secciones
              </span>
            </div>
          ...
          <div className="space-y-4">
            {sections.map((section) => {
              ...
              const toggleSection = () => {
                setExpandedSections((prev: number[]) => {
                  ...
                  if (isExpanding) {
                    ...
                    const subKeys = section.subSecciones.map(
                      (sub) => `${section.id}-${sub.id}`
                    );
                    setExpandedSubSections((current: string[]) =>
                      Array.from(new Set([...current, ...subKeys]))
                    );
                  }
                  return isExpanding
                    ? [...prev, section.id]
              ...
              return (
                <div
                  key={section.id}
                  className="bg-white rounded-xl shadow-sm border border-[#4790FD]/30 overflow-hidden transition-all duration-300"
                >
                  {/* Accordion Header */}
                  <div
                      ...
                      <div className="flex items-center gap-2 text-xs text-gray-400 mt-1 font-medium">
                        ...
                        <span>
                          {section.subSecciones.reduce(
                            (acc, sub) => acc + (sub.recurso?.length || 0),
                            0
                          )}{' '}
                          recursos
                        </span>
                      ...
                      <div className="space-y-3">
                        {section.subSecciones.length > 0 ? (
                          section.subSecciones.map((sub) => {
                            ...
                            const isSubExpanded =
                              expandedSubSections.includes(subKey);
                                ...
                                >
                                  <div className="flex flex-col text-left">
                                    ...
                                    <p className="text-[11px] text-gray-400 font-medium">
                                      {sub.recurso?.length || 0} documentos
                                      disponibles
                                    </p>
                                  </div>
                                  <div className="flex-shrink-0 ml-4">
                                  ...
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 animate-fadeIn pb-6">
                                    {sub.recurso && sub.recurso.length > 0 ? (
                                      sub.recurso.map((rec, idx) => (
                                        <div
                                          key={idx}
                                          className="bg-white rounded-xl overflow-hidden border border-[#d1e7dd] hover:shadow-md transition-all flex flex-col group"
                                        >
                                          ...
                                          <div className="relative h-40 md:h-48 w-full bg-gray-50 overflow-hidden border-b border-gray-100">
                                            ...
                                            <img
                                              src={
                                                rec.imagen ||
                                                'https://ui-avatars.com/api/?name=PDF&background=4790FD&color=fff'
                                              }
                                              alt={
                                                rec.nombreArchivo || 'Recurso'
                                              }
                                          ...
                                          <div className="p-4 flex-1 flex flex-col">
                                            <div className="mb-3">
                                              <span className="bg-[#d1e7dd] text-[#0f5132] text-[10px] font-bold px-2 py-0.5 rounded-full uppercase">
                                                Libre
                                              </span>
                                            </div>
                                            ...
                                            <div className="mt-auto flex gap-2">
                                              <button
                                                onClick={() =>
                                                  window.open(rec.pdf, '_blank')
                                                }
                                                className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 border border-[#4790FD] text-[#4790FD] text-[11px] font-bold rounded-lg transition-all hover:bg-[#4790FD] hover:text-white ${
                                                  !rec.pdf
                                              ...
                                              <button
                                                ...
                                                className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-[#002855] text-white text-[11px] font-bold rounded-lg transition-all hover:bg-[#001d3d] ${
                                                  !rec.pdf
                                                    ? 'opacity-50 cursor-not-allowed'
                                                    : ''
                                                }`}
                                      ...
                                      <p className="col-span-full text-center text-gray-400 py-4 italic text-sm">
